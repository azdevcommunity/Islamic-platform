...
/**
 * Mock data for video slides
 * @type {VideoSlide[]}
 */
export const videoSlidesMock: VideoSlide[] = [
  {
    id: 1,
    videoSrc: "https://res.cloudinary.com/dhhlnrons/video/upload/v1743080791/esm/homepage/lubjcf8kkdivjgk2hdox.mp4",
    title: "Ramazan Avari",
    youtubeLink: process.env.NEXT_PUBLIC_YTB_CHANNEL_URL || "https://youtube.com",
  },
];

/**
 * Islamic Video Slider Component
...
export default function IslamicVideoSlider() {
  ...
  if (!videoSlidesMock.length) return null;
  ...
  return (
    <Swiper
      ...
      aria-label="İslami video slayder"
    >
      {videoSlidesMock.map((slide, index) => (
        <SwiperSlide key={slide.id} className="relative h-full w-full">
          ...
          <video
            ...
            aria-label={`Video: ${slide.title} - İslami təqdimat`}
          >
          ...
          <div className="absolute inset-0 flex items-center justify-center lg:justify-start text-white bg-gradient-to-r from-black/70 via-black/50 to-transparent p-6 md:p-10 lg:p-16">
            ...
            <a
              ...
              aria-label={`"${slide.title}" videosunu YouTube-da izləyin`}
              className="absolute top-6 right-6 z-20 flex items-center gap-2 bg-white/10 backdrop-blur-md text-white px-4 py-3 rounded-xl hover:bg-white/20 transition-all duration-300 border border-white/20 focus:outline-none focus:ring-2 focus:ring-white/50"
            >
              <FaYoutube className="h-5 w-5 text-red-500" aria-hidden="true" />
            ...
            <div className="text-center lg:text-left space-y-6 max-w-2xl lg:max-w-3xl">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4 opacity-0 animate-fade-in-up animate-delay-800">
                <Link
                  href="/articles"
                  className="inline-flex items-center justify-center px-8 py-4 bg-primary-600 text-white font-semibold rounded-xl hover:bg-primary-700 transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2"
                >
                  Məqalələri Oxu
                </Link>
                <Link
                  href="/videos"
                  className="inline-flex items-center justify-center px-8 py-4 bg-white/10 backdrop-blur-sm text-white font-semibold rounded-xl hover:bg-white/20 transition-all duration-300 border border-white/30 focus:outline-none focus:ring-2 focus:ring-white/50"
                >
                  Videoları Kəşf Et
                </Link>
              </div>
