...
const PlaylistsGrid = ({playlistId, search, videoId, content}) => {
    ...
    useEffect(() => {
        const fetchPlaylists = async () => {
            try {
                ...
                let playlistsData = await res.json()
    ...
    useEffect(() => {
        if (playlists.length > 0 && playlistId) {
            const sortedPlaylists = [...playlists].sort((a, b) => 
                (a.playlistId === playlistId ? -1 : b.playlistId === playlistId ? 1 : 0)
            )
    ...
    // Sort playlists to put the selected one first
    const sortedPlaylists = playlistId 
        ? [...playlists].sort((a, b) => (a.playlistId === playlistId ? -1 : b.playlistId === playlistId ? 1 : 0))
        : playlists
    ...
    const buildPageLink = (dynamicPlaylistId) => {
        ...
        // Playlist kartına klik etdikdə video player açılsın, playlist siyahısı deyil
        // content parametrini göndərmirik ki, default olaraq video player açılsın
