...
import { getHighQualityThumbnail, getLowQualityThumbnail, getThumbnailPair, ThumbnailManager } from "@/util/Thumbnail.js";
...
interface VideosGridServerProps {
    videos: Video[];
    totalPages: number;
    content: string;
    search: string;
...
export default async function VideosGridServer({
    videos,
    totalPages,
    content,
    search,
    page,
}: VideosGridServerProps) {
        ...
        <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {await Promise.all(videos.map(async (video, index) => {
                    const src = getHighQualityThumbnail(video.thumbnail);
                    const lowQualitySrc = getLowQualityThumbnail(video.thumbnail);

                    // Sadə SVG blur istifadə et (sürətli və kiçik)
